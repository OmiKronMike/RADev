...
@FileSystemInfo(type = "fstypegoeshere", // ([a-z0-9]+ only)
		description = "File system description goes here", factory = small-soldiersFileSystem.MyFileSystemFactory.class)
public class small-soldiersFileSystem implements GFileSystem {
	 ...
	 */
	public small-soldiersFileSystem(FSRLRoot fsFSRL, ByteProvider provider) {
		this.fsFSRL = fsFSRL;
	...
	public void mount(TaskMonitor monitor) {
		monitor.setMessage("Opening " + small-soldiersFileSystem.class.getSimpleName() + "...");
	...
	public static class MyFileSystemFactory
			implements GFileSystemFactoryByteProvider<small-soldiersFileSystem>,
			GFileSystemProbeByteProvider {

		@Override
		public small-soldiersFileSystem create(FSRLRoot targetFSRL,
				ByteProvider byteProvider, FileSystemService fsService, TaskMonitor monitor)
				throws IOException, CancelledException {

			small-soldiersFileSystem fs = new small-soldiersFileSystem(targetFSRL, byteProvider);
			fs.mount(monitor);
			return fs;
		}
